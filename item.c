...
int esta_valido(Item item)
{
    ...
    {
        fprintf(stderr, "Data inv�lida: %s\n", item.data_entrada);
        return 0;
    }
...
{
    if (strlen(localizacao) != 3)
        return 0;
    if (localizacao[0] < 'A' || localizacao[0] > 'Z')
        return 0;
    if (localizacao[1] < 'A' || localizacao[1] > 'Z')
        return 0;
    if (localizacao[2] < '0' || localizacao[2] > '9')
        return 0;
    return 1;
}
